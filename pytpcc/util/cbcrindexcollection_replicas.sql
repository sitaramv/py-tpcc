...
CREATE INDEX CU_W_ID_D_ID_LAST on CUSTOMER(C_W_ID, C_D_ID, C_LAST) PARTITION BY HASH (C_W_ID, C_D_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
CREATE INDEX DI_ID_W_ID on DISTRICT(D_ID, D_W_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
CREATE INDEX D_ID_W_ID_O_ID on NEW_ORDER(NO_D_ID, NO_W_ID, NO_O_ID) PARTITION BY HASH (NO_D_ID, NO_W_ID, NO_O_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
CREATE INDEX OR_DID_WID_ID_CID on ORDERS(O_D_ID, O_W_ID, O_ID, O_C_ID) PARTITION BY HASH (O_D_ID, O_W_ID, O_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
CREATE INDEX OR_CID_DID_WID_ID_CAID_EID on ORDERS(O_C_ID, O_D_ID, O_W_ID, O_ID DESC, O_CARRIER_ID, O_ENTRY_D) PARTITION BY HASH (O_C_ID, O_D_ID, O_W_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
CREATE INDEX OL_O_ID_D_ID_W_ID on ORDER_LINE(OL_O_ID, OL_D_ID, OL_W_ID) PARTITION BY HASH (OL_O_ID, OL_D_ID, OL_W_ID) USING GSI WITH {"defer_build": true, "num_replica":replicas}
